//
//  Constants.m
//  FBCheckIn
//
//  Created by Teja on 1/3/13.
//
//

#import "Constants.h"

NSString * const kTopPicksFilter = @"Top picks";
NSString * const kFoodFilter = @"Food";
NSString * const kCoffeeFilter = @"Coffee";
NSString * const kNightLifeFilter = @"Bars & Nightlife";
NSString * const kArtsFilter = @"Arts";
NSString * const kShoppingFilter = @"Shopping";
NSString * const kSightsFilter = @"Sights";
NSString * const kEventsFilter = @"Nearby Events";
NSString * const kFriendsBeenToFilter = @"Friends have been to";
